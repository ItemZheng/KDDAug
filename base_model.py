...
class BaseModel(nn.Module):
    ...
    def forward(self, v, q, labels, bias,v_mask, return_feat=False):
        """Forward
        ...
        if labels is not None:
            # sig_logits = logits.softmax(dim=1)
            # kl_loss = (-labels * torch.log(sig_logits)).sum(dim=1).mean()
        ...
        if return_feat:
            # joint_repr = self.proj_head(joint_repr)
            return logits, loss, w_emb, joint_repr
...
def build_baseline0_newatt(dataset, num_hid):
    ...
    # proj = FCNet([num_hid, num_hid, num_hid])
    return BaseModel(w_emb, q_emb, v_att, q_net, v_net, classifier)